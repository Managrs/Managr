...
import FreelancerDashboard from '../../src/components/ClientDashboard.vue' // adjust if path or filename differs

describe('FreelancerDashboard.vue', () => {
  it('renders search header', () => {
    const wrapper = mount(FreelancerDashboard)
  ...
  it('shows "no freelancers" if no matches', async () => {
    const wrapper = mount(FreelancerDashboard)

    await wrapper.find('.search-bar').setValue('Nonexistent Skill')
    await wrapper.find('#search-btn').trigger('click')
