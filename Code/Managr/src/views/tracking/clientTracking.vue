    ...
    <section class="projects-container">
      ...
      >
        ...
        <section class="payment-section" v-if="project.totalPrice - project.paidAmount > 0">
          ...
          <fieldset class="payment-form">
            ...
            <input 
              id="payment-amount"
              type="number" 
              :max="project.totalPrice - project.paidAmount"
              min="0"
              step="0.01"
              v-model="project.paymentAmount"
              class="payment-input"
            ...
            <select 
              id="payment-method"
              v-model="project.selectedPaymentMethod"
          ...
          <button 
            @click="makePayment(project.id)"
            :disabled="!canMakePayment(project)"
            class="payment-btn"
          >
        ...
        <section class="communication-section">
          ...
          <button 
            @click="contactFreelancer(project.id)"
            class="contact-btn"
          >
            Contact {{ project.freelancer }}
          </button>
          <button 
            @click="requestUpdate(project.id)"
            class="update-btn"
          >
...
export default {
  ...
  methods: {
    ...
    contactFreelancer(projectId) {
      ...
      console.log(`Contacting ${project.freelancer} for project ${projectId}`);
      // Here you would open messaging interface
      alert(`Opening message thread with ${project.freelancer}`);
    },
    
    requestUpdate(projectId) {
      console.log(`Requesting update for project ${projectId}`);
      alert('Progress update request sent to freelancer');
    },
    ...
    downloadInvoice(projectId) {
      console.log(`Downloading invoice for project ${projectId}`);
...
.payment-section, .payment-status-section {
  margin-bottom: 2rem;
  padding: 1.5rem;
...
.payment-section h3, .payment-status-section h3 {
  color: var(--orange-accent);
  margin-bottom: 1rem;
}
...
.contact-btn:hover, .update-btn:hover {
  background: var(--navy-accent);
  color: var(--primary-white);
}
...
.modal-content {
  background: #ffffff; /* Bright modal */
  border-radius: 12px;
  padding: 2rem;
  max-width: 500px;
  width: 90%;
  box-shadow: 0 15px 40px rgba(0, 0, 0, 0.35); /* Stronger shadow */
  border: 1px solid var(--border-gray); /* Optional subtle border */
}
